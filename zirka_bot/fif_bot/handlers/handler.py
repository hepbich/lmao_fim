...
from zirka_bot import (MESSAGES, REPLICAS, USERS, WORKS_CHATS,
                         BOT_FIRST_NAME, RATING_LIMIT, FLOOD_RATE, L_USERS)
from zirka_bot.dispatcher import dp
from zirka_bot.handlers.controllers import (flood_controlling, get_answer,
                                              get_ai_answer)
...
@dp.message_handler(Text(contains=['Привет', 'C чего начать'],
                         ignore_case=True))
...
@dp.message_handler(Text(contains=['C чего начать'], ignore_case=True))
...
@dp.message_handler(Text(contains=['https://t.me/Boyfromgreenland'],
                         ignore_case=True))
async def bar_reply(message: types.Message):
    await message.reply(choice(REPLICAS['tg_link_author']))
    ...
    if message.text.startswith(BOT_FIRST_NAME):
        if str(message.chat.id) in WORKS_CHATS:
            if username in L_USERS:
                await message.reply(choice(REPLICAS['Waiting_Zirka']))
